.parse_spatial_params <- function(params, num_genes, phyla, is_discrete) {
  ...
  cell_type_map <- NULL
  cell_type_factors <- if (is.data.frame(ctp)) {
    if (!all(sort(paste0(ctp0[,1], ctp0[,2], sep = "%%")) ==
             sort(paste0(ctp[,1], ctp[,2], sep = "%%")))) {
      stop("invalid cell.type.interaction. Please use cci_cell_type_params()") 
    }
    .cell_type_param_to_matrix(ctp)
...
cci_cell_type_params <- function(tree, total.lr, ctype.lr, step.size = 1, rand = T, discrete = F) {
  ...
  res <- array(0, dim = c(n, n, total.lr, total.lr))
...
CreateSpatialGrid <- function(ncells, max_nbs, .same.type.prob = 0.8) {
  grid_size <- ceiling(sqrt(ncells) * 1.5)
