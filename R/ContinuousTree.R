...
ImpulseEVFpertip <- function(phyla, edges, root, tips, internal, neutral, tip, Sigma, evf_center, step_size) {
  beta <- runif(1, 5, 15)
  h1 <- runif(1, 2, 5)
  h2 <- runif(1, 0, h1)
  t1 <- runif(1, 0, 0.9)
  ...
  tiplabels(cex = 2)
  nodelabels(cex = 2)
  neutral2 <- SampleSubtree(root, 0, evf_center, edges, ncells, step_size, neutral)
