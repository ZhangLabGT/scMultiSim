...
.get_result <- function(sim, do_velocity, is_debug) {
  cell_meta <- if (sim$do_spatial) {
    ...
    cbind(
      cell_id = paste("cell", 1:sim$N$cell, sep = "_"),
      sim$CIF_all$meta
    )
  }
  ...
  if (sim$do_spatial) {
    ...
    for (i in seq_along(sim$sp_regulators)) {
      tg <- sim$sp_targets[i]
      rg <- sim$sp_regulators[i]
      for (ct1 in sim$cell_type_map) {
        for (ct2 in sim$cell_type_map) {
          if (sim$sp_ctype_param[ct1, ct2, i, i] > 0) {
            ctype_param <- rbind(ctype_param, data.frame(
              ligand = rg, receptor = tg,
              effect = sim$sp_effect[tg, rg],
              ct1 = ct1, ct2 = ct2
            ))
          }
        }
...
#' @return list
.normalize_GRN_params <- function(GRN_params) {
  if (!is.data.frame(GRN_params)) {
    return(NULL)
  }

  orig_gene_ids <- unique(c(GRN_params[, 1], GRN_params[, 2])) %>%
    sort() %>%
    as.character()
  ...
  # rename gene ids
  for (j in c(1, 2)) {
    for (i in seq_len(nrow(GRN_params))) {
      GRN_params[i, j] <- name_map[[as.character(GRN_params[i, j])]]
    }
  }

  # get new target and regulator list
  targets <- sort(unique(GRN_params[, 1]))
  regulators <- sort(unique(GRN_params[, 2]))
