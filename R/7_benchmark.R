...
plot_tsne <- function(data, labels, perplexity = 60, legend = '', plot.name = '', save = F, rand.seed = 0) {
  ...
  if (save) {
    ggsave(p, filename = paste0(plot.name, '.pdf'), device = 'pdf', width = 5, height = 5)
    return(NULL)
  } else {
...
gene_corr_cci <- function(
  ...
  if (all.genes) {
    ...
    res <- data.frame(target, regulator, correlation, corr.non.nbs)  
    if (is.null(.pair)) {
      res
    } else {
      res_pair 
...
plot_rna_velocity <- function(
  ...
  perplexity = 70, arrow.length = 1, saving = F, randseed = 0, ...
) {
  set.seed(randseed)
  ...
  args <- default.args(width = 20*2/3, height = 10, units = "in", dpi = 1000)
  ...
  for (i in seq_along(types)) {
    ...
    if (saving) {
      ggsave(filename = "velocity_" %+% .type %+% ".pdf", plot = .plot,
...
rna_velocity_cosine <- function(
  ...
  sapply(1:80, function(i) cos.sim(gt_velo_mean[i,], res_velo_mean[i,]))
